...
- **`backend-deploy.yml`** : build Maven + tests → build image Docker → push sur Artifact Registry → déploiement Cloud Run ( à configurer)
- **`frontend-deploy.yml`** : `npm run build` (vérifie aussi les types TS) → déploiement Firebase Hosting (à configurer)
